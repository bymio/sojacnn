<template>
  <div>
    <el-card>
      <el-tabs v-model="getActiveName.contypeId" @tab-click="handleClick">
...
<script>
import { findContype ,findCategoryByContypeId ,updateNewsBie,removeNewsBie } from "@/api/index";
export default {
  name: "newsList4",
  components: {},
  data() {
  ...
  methods:{
    handleClick(tab,event) {//点击每个新闻类型触发
      this.getActiveName.contypeId = tab.name
      // console.log(this.getActiveName.contypeId)
      // 调用获取新闻类别函数
      this.getNewsBie()
    },
    async findNewsXing() {
    ...
    async getNewsBie(){//根据新闻类型id获取类别
      const res = await findCategoryByContypeId(this.getActiveName)
